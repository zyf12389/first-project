#ifndef FASTGLOBALSMOOTHFILTWE_H
#define FASTGLOBALSMOOTHFILTWE_H
#include<iostream>
#include"fastGlobalSmoothFilter.h"
using namespace std;
using namespace cv;
...
{
public:
	virtual void filter(InputArray disparity_map_left, InputArray left_view, OutputArray filtered_disparity_map,OutputArray filter_right, InputArray disparity_map_right, Rect ROI, InputArray)=0;
	virtual Mat getConfidenceMap() = 0;
	virtual void setConfidenceMap(Mat conf) = 0;
	virtual Mat getConfidenceMap_right() = 0;
	virtual void setConfidenceMap_right(Mat conf) = 0;
	virtual int getDepthDiscontinuityRadius() = 0;
	virtual double getLambda() = 0;
	virtual int getIter() = 0;
	virtual double getLambda_a() = 0;
	...
	virtual void setLambda_a(double lambda_a) = 0;	
	virtual void setMultiple(int multiple) = 0;
	virtual int getMultiple() = 0;
};
...
protected:
	...
	bool use_confidence;
	Mat confidence_map;
	Mat confidence_map_right;

	double lambda, sigma_color,lambda_a;
	int iter;
	...
	int num_stripes;
	int multiple;
	void init(double _lambda, double _sigma_color,int _iter,double _lambda_a,bool _use_confidence, int l_offs, int r_offs, int t_offs, int b_offs, int _min_disp);
...
protected:
	struct ComputeDiscontinuityAwareLRC_ParBody : public ParallelLoopBody
	{
		...
		Mat *left_disc, *right_disc, *dst,*right_dst;
		Rect left_ROI, right_ROI;
		int nstripes, stripe_sz;

		ComputeDiscontinuityAwareLRC_ParBody(DisparityWLSFilterImpl& _wls, Mat& _left_disp, Mat& _right_disp, Mat& _left_disc, Mat& _right_disc, Mat& _dst, Mat& _right_dst,Rect _left_ROI, Rect _right_ROI, int _nstripes);
...
public:
	...
	void filter(InputArray disparity_map_left, InputArray left_view, OutputArray filtered_disparity_map,OutputArray filter_right, InputArray disparity_map_right, Rect ROI, InputArray right_view);
	void filter_(InputArray disparity_map_left, InputArray left_view, OutputArray filtered_disparity_map, OutputArray filter_right,InputArray disparity_map_right, Rect ROI, InputArray right_view);
	...
	double getLambda_a(){ return lambda_a; }
	void setLambda_a(double _lambda_a){ lambda_a = _lambda_a; }
	...
	void setConfidenceMap(Mat conf){ conf.copyTo(confidence_map); }
	Mat getConfidenceMap_right() { return confidence_map_right; }
	void setConfidenceMap_right(Mat conf){ conf.copyTo(confidence_map_right); }
	void setMultiple(int multi){ multiple = multi; }
	int getMultiple(){ return multiple; }
	Rect getROI() { return valid_disp_ROI; }
};

void DisparityWLSFilterImpl::init(double _lambda, double _sigma_color, int _iter,double _lambda_a,bool _use_confidence, int l_offs, int r_offs, int t_offs, int b_offs, int _min_disp)
{
...
void DisparityWLSFilterImpl::computeDepthDiscontinuityMaps(Mat& left_disp, Mat& right_disp, Mat& left_dst, Mat& right_dst)
{
	...
	imshow("ldiscontinuity", left_dst_ROI);
	imshow("rdiscontinuity", right_dst_ROI);
}
...
void DisparityWLSFilterImpl::computeConfidenceMap(InputArray left_disp, InputArray right_disp)
{
	...
	confidence_map_right = depth_discontinuity_map_right;

	parallel_for_(Range(0, num_stripes), ComputeDiscontinuityAwareLRC_ParBody(*this, ldisp, rdisp, depth_discontinuity_map_left, depth_discontinuity_map_right, confidence_map, confidence_map_right,valid_disp_ROI, right_view_valid_disp_ROI, num_stripes));
	confidence_map = 255.0f*confidence_map;
	confidence_map_right = 255.0f*confidence_map_right;
	imshow("confidencemap", confidence_map);
	imshow("confidencemap_right", confidence_map_right);
}
...
Ptr<DisparityWLSFilterImpl> DisparityWLSFilterImpl::create(bool _use_confidence, int l_offs = 0, int r_offs = 0, int t_offs = 0, int b_offs = 0, int min_disp = 0)
{
	...
	wls->init(8000.0, 1.0,1,1.0, _use_confidence, l_offs, r_offs, t_offs, b_offs, min_disp);
	return Ptr<DisparityWLSFilterImpl>(wls);
}

void DisparityWLSFilterImpl::filter(InputArray disparity_map_left, InputArray left_view, OutputArray filtered_disparity_map,OutputArray filtered_right, InputArray disparity_map_right, Rect ROI, InputArray right_view)
{
	Mat left = disparity_map_left.getMat();
	Mat right = disparity_map_right.getMat();
	if (left.type() == CV_32F&&right.type()==CV_32F){
		filter_(disparity_map_left, left_view, filtered_disparity_map,filtered_right, disparity_map_right, ROI,right_view);
		
	}
	else {
		Mat left_c, right_c, filt_disp,filt_disp_right;
		left.convertTo(left_c, CV_32F);
		right.convertTo(right_c, CV_32F);
		filter_(left_c, left_view, filt_disp,filt_disp_right, right_c, ROI,right_view);
		filt_disp.convertTo(filtered_disparity_map, CV_32F);
		filt_disp_right.convertTo(filtered_right, CV_32F);
	}

}

void DisparityWLSFilterImpl::filter_(InputArray disparity_map_left, InputArray left_view, OutputArray filtered_disparity_map, OutputArray filter_right,InputArray disparity_map_right, Rect ROI, InputArray right_view)
{
	CV_Assert(!disparity_map_left.empty() && (disparity_map_left.depth() == CV_32F) && (disparity_map_left.channels() == 1));
	if (!left_view.empty())
		CV_Assert((left_view.depth() == CV_8U) && (left_view.channels() == 3 || left_view.channels() == 1));//
	if (!right_view.empty())
		CV_Assert((right_view.depth() == CV_8U) && (right_view.channels() == 3 || right_view.channels() == 1));//
	Mat disp, src, src_right,dst,disp_right,dst_right;
	if (disparity_map_left.size() != left_view.size())
		resize_factor = disparity_map_left.size().width / (float)left_view.size().width;
	else
		resize_factor = 1.0;
	if (ROI.area() != 0) /* user provided a ROI */
		...
		disparity_map_left.size().width- left_offset - right_offset,
		disparity_map_left.size().height - top_offset - bottom_offset);
	...
	if (!use_confidence)
	{
		...
		Mat filtered_disp;
		fastGlobalSmootherFilter(src, disp, filtered_disp, lambda, sigma_color,lambda_a,iter);
		filtered_disp.copyTo(dst);
	}
	else
	{
		CV_Assert(!disparity_map_right.empty() && (disparity_map_right.depth() == CV_32F) && (disparity_map_right.channels() == 1));//
		CV_Assert((disparity_map_left.size().width == disparity_map_right.size().width));
		CV_Assert((disparity_map_left.size().height == disparity_map_right.size().height));
	//	computeConfidenceMap(disparity_map_left, disparity_map_right);
		Mat disp_full_size = disparity_map_left.getMat();
		Mat disp_full_size_right = disparity_map_right.getMat();
		Mat src_full_size = left_view.getMat();
		if (disp_full_size.size != src_full_size.size)
		{
			float x_ratio = src_full_size.cols / (float)disp_full_size.cols;
			...
			ROI = valid_disp_ROI;
		disp = Mat(disp_full_size, ROI);
		disp_right = Mat(disp_full_size_right, ROI);
		src = Mat(src_full_size, ROI);
		filtered_disparity_map.create(disp_full_size.size(), disp_full_size.type());
		Mat& dst_full_size = filtered_disparity_map.getMatRef();
		filter_right.create(disp_full_size_right.size(), disp_full_size_right.type());
		Mat &dst_full_size_right = filter_right.getMatRef();
	//	dst_full_size = Scalar(multiple * (min_disp ));
		dst = Mat(dst_full_size, ROI);
		dst_right = Mat(dst_full_size_right, ROI);
		Mat conf(confidence_map, ROI);
		Mat conf_right(confidence_map_right, ROI);

		Mat disp_mul_conf,disp_mul_conf_right;
		disp_mul_conf = conf.mul(disp);
		disp_mul_conf_right = conf_right.mul(disp_right);

		Mat conf_filtered,conf_filtered_right;
		Ptr<FastGlobalSmootherFilter> wls = createFastGlobalSmootherFilter(src, lambda, sigma_color,lambda_a,iter);
		wls->filter(disp_mul_conf, disp_mul_conf);
		wls->filter(conf, conf_filtered);
		dst = disp_mul_conf.mul(1 / (conf_filtered + EPS));
		if (!right_view.empty())
		{
			Mat src_full_size_right = right_view.getMat();
			src_right = Mat(src_full_size_right, ROI);
			Ptr<FastGlobalSmootherFilter> wls_right = createFastGlobalSmootherFilter(src_right, lambda, sigma_color, lambda_a, iter);
			wls_right->filter(disp_mul_conf_right, disp_mul_conf_right);
			wls_right->filter(conf_right, conf_filtered_right);
			dst_right = disp_mul_conf_right.mul(1 / (conf_filtered_right + EPS));
		}	
	}
}

DisparityWLSFilterImpl::ComputeDiscontinuityAwareLRC_ParBody::ComputeDiscontinuityAwareLRC_ParBody(DisparityWLSFilterImpl& _wls, Mat& _left_disp, Mat& _right_disp, Mat& _left_disc, Mat& _right_disc, Mat& _dst,Mat& _right_dst, Rect _left_ROI, Rect _right_ROI, int _nstripes) :
wls(&_wls), left_disp(&_left_disp), right_disp(&_right_disp), left_disc(&_left_disc), right_disc(&_right_disc), dst(&_dst), right_dst(&_right_dst),left_ROI(_left_ROI), right_ROI(_right_ROI), nstripes(_nstripes)
{
...
void DisparityWLSFilterImpl::ComputeDiscontinuityAwareLRC_ParBody::operator() (const Range& range) const
{
	...
	for (int i = start; i<end; i++)
	{
		...
		for (int j = j_start2; j < right_end; j++)
		{
			left_idx = j + ((int)row_right[j] / wls->multiple);
			if (left_idx >= left_ROI.x&&left_idx < j_end)
			{
				if (abs(row_right[j] - row_left[left_idx])< thresh)
					row_dst_right[j] = min(row_right_conf[j], row_left_conf[left_idx]);
				else
					row_dst_right[j] = 0.0f;
			}
		}
	}
}
...
double computeMSE(InputArray GT, InputArray src, Rect ROI)
{
	...
	CV_Assert(src.size().height == GT.size().height && src.size().width == GT.size().width);
	double res = 0;
