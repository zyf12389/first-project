...
void costAggregationRight(const Mat& lImg, const Mat& rImg, const int maxDis, Mat* costVol)
{
	printf("\n\t\tNon-local cost aggregation");
	// construct non-local aggregation class
	...
	for (int d = 0; d < maxDis; d++) {
		...
		for (int y = 0; y < h; y++) {
			double* cost = (double*)costVol[d].ptr<double>(y);
			for (int x = 0; x < w; x++) {
				m_nlca.m_cost_vol_right[y][x][d] = cost[x];
			}
		}
	}
#endif
	...
	// filter cost volume
	m_nlca.m_tf_right.filter(m_nlca.m_cost_vol_right[0][0],
		m_nlca.m_cost_vol_temp[0][0],
		m_nlca.m_nr_plane
		);
	...
	for (int d = 1; d < maxDis; d++) {
		...
		for (int y = 0; y < h; y++) {
			double* cost = (double*)costVol[d].ptr<double>(y);
			for (int x = 0; x < w; x++) {
				cost[x] = m_nlca.m_cost_vol_right[y][x][d];
