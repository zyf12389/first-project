...
inline int qx_mst_compute_edges_4neighbor(int**edges,unsigned char*distance,unsigned char*image,int nr_channel,int h,int w)//虽然是4邻域但是只需要计算每个点的右和下边的排序是按行
{
	int y0,yt,x0,xt;
	int nr_edge=0;
	for(y0=0;y0<h;y0++)
...
inline int qx_mst_compute_edges_8neighbor(int**edges,unsigned char*distance,unsigned char*image,int nr_channel,int h,int w)//在4邻域基础上加上对角线
{
	int y0,yt,x0,xt;
